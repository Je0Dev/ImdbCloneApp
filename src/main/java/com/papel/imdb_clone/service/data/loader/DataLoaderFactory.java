...
public class DataLoaderFactory {
    ...
    //map of loaders for each type of data loader
    private final Map<Class<?>, Object> loaders = new HashMap<>();
    private final InMemoryUserRepository userRepository;//user repository
    private final InMemoryMovieRepository movieRepository;//movie repository
    private final InMemorySeriesRepository seriesRepository;//series repository
    private final MoviesService moviesService;//movies service
    private final SeriesService seriesService;//series service
    private final CelebrityService<Actor> actorService;//actor service
    private final CelebrityService<Director> directorService;//director service
     ...
     * @param celebritiesRepository the celebrities repository
    ...
    public DataLoaderFactory(
            ...
            InMemoryCelebritiesRepository celebritiesRepository,
    ...
    private void initializeLoader(Class<?> loaderClass) {
        if (loaderClass.equals(ActorDataLoader.class)) {
            ...
            //initialize user data loader
            loaders.put(loaderClass, new UserDataLoader(userRepository));
        } else if (loaderClass.equals(AwardsDataLoader.class)) {
            //initialize awards data loader
            loaders.put(loaderClass, new AwardsDataLoader(movieRepository, seriesRepository, moviesService, seriesService));
        } else {
