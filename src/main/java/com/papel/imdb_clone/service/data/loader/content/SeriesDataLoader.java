...
public class SeriesDataLoader extends BaseDataLoader {
    ...
    private final SeriesService seriesService; //service for series
    private final CelebrityService<Actor> actorService; //service for actor for seriees instance
    private final LocalDate birthDate = LocalDate.now().minusYears(30); // Default to 30 years ago for some distance
    private final Ethnicity ethnicity = Ethnicity.CAUCASOID; // Default ethnicity
    private final CelebrityService<Director> directorService; //service for director for series instance
    private String creatorName; //creator name of the series.same as director
    private Series series; // Will be initialized with proper parameters

    /**
     * Constructor for SeriesDataLoader.
     * @param seriesService service for series which is loaded
     * @param actorService service for actor which is creator of the series
     * @param directorService service for director which is creator of the series
     */
    public SeriesDataLoader(
            SeriesService seriesService,
            CelebrityService<Actor> actorService,
            CelebrityService<Director> directorService) {
        //initialize services to be used in this class
    ...
    public int load(String filename) throws IOException {
        ...
        try (InputStream inputStream = getResourceAsStream(filename);
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    if (parts.length >= 7) {
                        ...
                        String directorName = "";
                        try {
                            directorName = parts[6].trim(); // director name
            ...
            // Calculate duration it took to load the series data
    ...
    /**
     * Helper method to split a line into parts, handling quoted fields
    ...
    private static String getString(String genreItem) {
        ...
        normalizedGenre = switch (normalizedGenre) {
            case "SCIFI" -> "SCI_FI"; // Map to SCI_FI
            case "SCIFANTASY" -> "SCI_FI"; // Map to SCI_FI
            case "ACTION", "ADVENTURE" -> "ACTION"; // Both map to ACTION
            case "ROMANCE", "MUSICAL" -> "ROMANCE"; // Map musicals to romance
            case "WAR", "HISTORY" -> "HISTORY"; // Map war to history
