...
public class FileDataLoaderService implements DataLoaderService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(FileDataLoaderService.class);//logger
    private final DataLoaderFactory loaderFactory;//data loader factory
     ...
     * @param celebritiesRepository InMemoryCelebritiesRepository
    ...
    public FileDataLoaderService(
        ...
        //Initialize loader factory for current data
        this.loaderFactory = new DataLoaderFactory(
                userRepository,
                movieRepository,
                seriesRepository,
                celebritiesRepository, moviesService,
                seriesService,
                actorService,
                directorService
        );
    ...
    private String findDataDirectory() {
        // Check common locations for the data directory if data files are not found in the default locations like src/main/resources/data or target/classes/data
    ...
    /**
     * Loads all data files in the correct order.
     */
    @Override
    public void loadAllData() {
        //for logging time taken to load data
        ...
        if (!filesVerified) {
            logger.warn("Proceeding with data loading despite missing files. Some data may not be loaded correctly.");
        }

        int successCount = 0;
        int failureCount = 0;

        // Load data files in the correct order
        for (String[] task : loadTasks) {
            String dataType = task[0];
            String filename = task[1];
    ...
    private <T extends BaseDataLoader> void loadDataFile(String filename, Class<T> loaderClass, String dataType) throws IOException {
        ...
        try {
            ...
            try {
                ...
                //Invocation target exception means that the method we are trying to invoke has thrown an exception
